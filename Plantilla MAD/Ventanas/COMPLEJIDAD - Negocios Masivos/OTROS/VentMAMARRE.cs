...
{
    public partial class VentMAMARRE : Form
    {
        public VentMAMARRE()
        {
            InitializeComponent();
        ...
        private void btnPlanilla_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                range1.Value = "IMEI Antiguo";
                range2 = sheet.Range["B1"];
                range2.Value = "Móvil";
                range3 = sheet.Range["C1"];
                range3.Value = "IMEI Nuevo";
                range4 = sheet.Range["D1"];
                range4.Value = "Negocio";
                sheet.Columns.AutoFit();
                try
                {
                    book.SaveAs(@docsDir + "AMARREDESAMARRE_MASIVO_" + DateTime.Now.ToString("HHmmss_ddMMyy") + ".xlsx");
                    MessageBox.Show("Se guardó el archivo AMARREDESAMARRE_MASIVO_" + DateTime.Now.ToString("HHmmss_ddMMyy") + ".xlsx \n\nEn " + docsDir, "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch { }
                book.Close();
                app.Quit();
