...
{
    public partial class VentMCSS106 : Form
    {
        public VentMCSS106()
        {
            InitializeComponent();
        ...
        private void btnPlanilla_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                range2.Value = "Patente SIM";
                range3 = sheet.Range["C1"];
                range3.Value = "Cuenta";
                range4 = sheet.Range["D1"];
                range4.Value = "Código SIM";
                range4 = sheet.Range["E1"];
                range4.Value = "Valor";
                sheet.Columns.AutoFit();
                try
                {
                    book.SaveAs(@docsDir + "CSS106_MASIVO_" + DateTime.Now.ToString("HHmmss_ddMMyy") + ".xlsx");
                    MessageBox.Show("Se guardó el archivo CSS106_MASIVO_" + DateTime.Now.ToString("HHmmss_ddMMyy") + ".xlsx \n\nEn " + docsDir, "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch { }
                book.Close();
                app.Quit();
