...
{
    public partial class VentCS : Form
    {
        public VentCS()
        {
            InitializeComponent();
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            if (validacionesTexto())
            {
                ...
                for (int j = 0; j < dataCS.ColumnCount; j++)
                {
                    if (j == 0)
                    {
                        col = col + dataCS.Columns[j].HeaderText;
                    }

                    else
                    {
                        col = col + separador + dataCS.Columns[j].HeaderText;
                    }
                ...
                String datos = "";
                String[,] mat = new String[dataCS.RowCount, 3];
                ...
                for (int i = 0; i <= dataCS.RowCount - 1; i++)
                {
                    ...
                    if (dataCS["Movil", i].Value != null)
                    {
                        if (dataCS["Cuenta", i].Value != null)
                        {
                            filacuenta = "Cuenta: " + dataCS["Cuenta", i].Value.ToString() + "\n";
                        }
                        if (dataCS["SIM", i].Value != null)
                        {
                            filasim = "SIM: " + dataCS["SIM", i].Value.ToString() + "\n";
                        }
                        if (dataCS["Valor", i].Value != null)
                        {
                            filavalor = "Valor: $" + dataCS["Valor", i].Value.ToString() + "\n" + "Móviles: \n";
                        }
                        if (dataCS["Cuenta", i].Value != null)
                        {
                            mat[i, 2] = dataCS["Cuenta", i].Value.ToString();
                            cuentas.Add(dataCS["Cuenta", i].Value.ToString());
                        }

                        mat[i, 0] = dataCS["Movil", i].Value.ToString();
...
if (comboEjecutivo.Text.Equals("Ximena Valdes V.")) {datosEjec = "\nAnexo: 0" + "\nMóvil: 999687998" + "\nEmail: XVALDES@ENTEL.CL";}
        ...
        private void btnCopiaProy_Click(object sender, EventArgs e)
        {
            ...
            switch (distinctcuentas.Count())
            {
                ...
                case 1:
                    labelCS.Text = "CS";
                    MessageBox.Show("Se copió el nombre de proyecto " + labelCS.Text + ".", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Clipboard.SetDataObject(labelCS.Text);
                    break;
                case 2:
                    labelCS.Text = "CS2";
                    MessageBox.Show("Se copió el nombre de proyecto " + labelCS.Text + ".", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnCopiaCuentas_Click(object sender, EventArgs e)
        {
            ...
            for (int i = totalfilas - 1; i >= 0; i--)
            {
                if (dataCS["Cuenta", i].Value != null)
                {
                    ult = dataCS["Cuenta", i].Value.ToString();
                    break;
                }
            }
        ...
        private void btnCopiaSIM_Click(object sender, EventArgs e)
        {
            ...
            for (int i = totalfilas - 1; i >= 0; i--)
            {
                if (dataCS["SIM", i].Value != null)
                {
                    ult = dataCS["SIM", i].Value.ToString();
                    break;
                }
            }
            for (int i = 1; i <= dataCS.RowCount - 1; i++)
        ...
        private void btnCopiaValor_Click(object sender, EventArgs e)
        {
            ...
            for (int i = totalfilas - 1; i >= 0; i--)
            {
                if (dataCS["Valor", i].Value != null)
                {
                    ult = dataCS["Valor", i].Value.ToString();
                    break;
                }
            }
            for (int i = 1; i <= dataCS.RowCount - 1; i++)
