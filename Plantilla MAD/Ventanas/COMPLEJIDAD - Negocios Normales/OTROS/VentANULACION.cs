...
{
    public partial class VentANULACION : Form
    {
        public VentANULACION()
        {
            InitializeComponent();
        }
        ...
        private void btnCopiaFolio_Click(object sender, EventArgs e)
        {
            String ult = "";
            ...
            for (int i = totalfilas - 1; i >= 0; i--)
            {
                ...
                {
                    ult = dataDatos["Factura", i].Value.ToString();
                    break;
                }
            }
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            if (validacionesTexto())
            {
                ...
                for (int i = 0; i <= dataDatos.RowCount - 1; i++)
                {
                    if (dataDatos["Factura", i].Value != null)
                    {
                        facturas.Add(dataDatos["Factura", i].Value.ToString());
                    }
                }
                ...
                foreach (String elemento in distinctfacturas)
                {
                    ...
                    for (int i = 0; i <= dataDatos.RowCount - 1; i++)
                    {
                        if ((dataDatos["Factura", i].Value != null) && (dataDatos["Factura", i].Value.ToString().Equals(elemento)))
                        {
                            if ((dataDatos["Serie", i].Value != null))
                            {
                                acum = acum + dataDatos["Serie", i].Value.ToString() + "\n";
                            }
                        }
                    }
                    datos = datos + "Documento: " + elemento + "\n" + acum + "\n";
                }
