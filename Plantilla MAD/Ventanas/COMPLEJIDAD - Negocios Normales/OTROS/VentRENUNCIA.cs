...
{
    public partial class VentRENUNCIA : Form
    {
        public VentRENUNCIA()
        {
            InitializeComponent();
        ...
        private void btnCopiaProy_Click(object sender, EventArgs e)
        {
            ...
            if (serie == false)
            {
                lblProy.Text = "RENUNCIA ACELERACION MULTA";
            }
            else
            {
                lblProy.Text = "RENUNCIA";
            ...
            if (validacionesTexto())
            {
                ...
                if (serie == false)
                {
                    foreach (String elemento in distinctcuentas)
                    {
                        ...
                        for (int i = 0; i <= dataDatos.RowCount - 1; i++)
                        {
                            if ((dataDatos["Cuenta", i].Value != null) && (dataDatos["Cuenta", i].Value.ToString().Equals(elemento)))
                            {
                                if ((dataDatos["Movil", i].Value != null))
                                {
                ...
                if (campoFactura.Text.Equals(""))
                {
                    texto = cabecera + datos + pie;
                }
                else
                {
                    texto = cabecera + datos + "Datos para factura: " + campoFactura.Text.Trim() + "\n\n" + pie;
                }
