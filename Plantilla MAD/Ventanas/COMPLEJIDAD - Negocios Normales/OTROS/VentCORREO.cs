...
{
    public partial class VentCORREO : Form
    {
        public VentCORREO()
        {
            InitializeComponent();
        ...
        private void VentCORREO_Load(object sender, EventArgs e)
        {
            radioMovil.Checked = true;
        }

        private void radioMovil_CheckedChanged(object sender, EventArgs e)
        {
            dataDatos.Columns["Cuenta"].Visible = false;
            dataDatos.Columns["Movil"].Visible = true;
            dataDatos.Columns["Movil"].Width = 100;
            btnCopiaBolsa.Location = new Point(330, 103);
            dataDatos.Size = new Size(392, 272);
            this.Size = new Size(638, 610);
            lblObs.Location = new Point(109, 411);
            campoObs.Location = new Point(112, 427);
            lblEjec.Location = new Point(347, 411);
            comboEjecutivo.Location = new Point(350, 427);
            btnCopiaProy.Location = new Point(354, 470);
            btnGrabar.Location = new Point(373, 518);
        }

        private void radioCuenta_CheckedChanged(object sender, EventArgs e)
        {
            dataDatos.Columns["Movil"].Visible = false;
            dataDatos.Columns["Cuenta"].Visible = true;
            dataDatos.Columns["Cuenta"].Width = 150;
            btnCopiaBolsa.Location = new Point(390, 103);
            dataDatos.Size = new Size(442, 272);
            this.Size = new Size(668, 610);
            lblObs.Location = new Point(129, 411);
            campoObs.Location = new Point(132, 427);
            lblEjec.Location = new Point(367, 411);
            comboEjecutivo.Location = new Point(370, 427);
            btnCopiaProy.Location = new Point(374, 470);
            btnGrabar.Location = new Point(393, 518);
        }
        ...
        private void btnCopiaBolsas_Click(object sender, EventArgs e)
        {
            String ult = "";
            int totalfilas = dataDatos.Rows.Count;
            if (radioMovil.Checked)
            {
                for (int i = totalfilas - 1; i >= 0; i--)
                {
                    if (dataDatos["Bolsa", i].Value != null)
                    {
                        ult = dataDatos["Bolsa", i].Value.ToString();
                        break;
                    }
                }
                for (int i = 1; i <= dataDatos.RowCount - 1; i++)
                {
                    if (dataDatos["Movil", i].Value != null)
                    {
                        if (dataDatos["Bolsa", i].Value == null)
                        {
                            dataDatos["Bolsa", i].Value = ult;
                        }
                    }
                }
            }
            else if (radioCuenta.Checked)
            {
                for (int i = totalfilas - 1; i >= 0; i--)
                {
                    if (dataDatos["Bolsa", i].Value != null)
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            if (validacionesTexto())
            {
                ...
                List<String> bolsas = new List<string>();
                ...
                for (int i = 0; i <= dataDatos.RowCount - 1; i++)
                {
                    if (dataDatos["Bolsa", i].Value != null)
                    {
                        bolsas.Add(dataDatos["Bolsa", i].Value.ToString());
                    }
                }

                IEnumerable<String> distinctbolsas = bolsas.AsQueryable().Distinct();
                String acum = "";
                foreach (String elemento in distinctbolsas)
                {
                    if (radioMovil.Checked)
                    {
                        acum = "Moviles: " + "\n";
                        for (int i = 0; i <= dataDatos.RowCount - 1; i++)
                        {
                            if ((dataDatos["Bolsa", i].Value != null) && (dataDatos["Bolsa", i].Value.ToString().Equals(elemento)))
                            {
                                if ((dataDatos["Movil", i].Value != null))
                                {
                   ...
                   else  if (radioCuenta.Checked)
                    {
                        ...
                        for (int i = 0; i <= dataDatos.RowCount - 1; i++)
                        {
                            if ((dataDatos["Bolsa", i].Value != null) && (dataDatos["Bolsa", i].Value.ToString().Equals(elemento)))
                            {
                                if ((dataDatos["Cuenta", i].Value != null))
                                {
                                    acum = acum + dataDatos["Cuenta", i].Value.ToString() + "\n";
                                }
                            }
                        }
                        datos = datos + "Bolsa: " + elemento + "\n" + acum + "\n";
