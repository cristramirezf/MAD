...
namespace Plantilla_MAD.Ventanas
{
    public partial class VentSERVICIOS : Form
    {
        ...
        private void btnCopiaServicios_Click(object sender, EventArgs e)
        {
            String ult = "";
            int totalfilas = dataDatos.Rows.Count;
            for (int i = totalfilas - 1; i >= 0; i--)
            {
                if (dataDatos["Servicios", i].Value != null)
                {
                    ult = dataDatos["Servicios", i].Value.ToString();
                    break;
                }
            }
            for (int i = 1; i <= dataDatos.RowCount - 1; i++)
            {
                if (dataDatos["Movil", i].Value != null)
                {
                    if (dataDatos["Servicios", i].Value == null)
                    {
                        dataDatos["Servicios", i].Value = ult;
                    }
                }
            }
        }
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            if (validacionesTexto())
            {
                ...
                for (int i = 0; i <= dataDatos.RowCount - 1; i++)
                {
                    if (dataDatos["Servicios", i].Value != null)
                    {
                        servicios.Add(dataDatos["Servicios", i].Value.ToString());
                    }
                }

                IEnumerable<String> distinctservicios = servicios.AsQueryable().Distinct();
                foreach (String elemento in distinctservicios)
                {
                    String acum = "Moviles: " + "\n";
                    for (int i = 0; i <= dataDatos.RowCount - 1; i++)
                    {
                        if ((dataDatos["Servicios", i].Value != null) && (dataDatos["Servicios", i].Value.ToString().Equals(elemento)))
                        {
                            if ((dataDatos["Movil", i].Value != null))
                            {
                    ...
                    datos = datos + "Servicios: " + elemento + "\n" + acum + "\n";
                }
